...
function send () {
    ...
    let days = Number(getValue("days"))
    let addCandles = Number(getValue("addCandlesMax"))
    let list = []
    let cheak = true
    let adds = false
    let beforeDay = days
        ...
        if (!isNum(nowCandles) || !isNum(candles) || !isNum(days) || nowCandles == "" || candles == "" || days == "" || (candles - nowCandles).toString().indexOf("-") != -1) {
            ...
            for (let i = ia; i < days; i++) {
                ...
                if (DayOfcandles + Number(ca) >= candles && !adds) {
                    ...
                    let nowtime = v.getFullYear() + "/" + String(v.getMonth() + 1) + "/" + String(v.getDate());
                    let now = `${nowtime.split(" ")[0]}`
                    result.innerHTML = "每天需要 " + a + " 或 " + DayOfcandles + "(主要) 根蠟燭 (比預計多 " + chDAY + " 天 - 預計到" + now + ")" + `<br>${list.join("<br>")}`
                    break;
                } else {
