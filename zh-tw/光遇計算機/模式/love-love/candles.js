        ...
        if (!isNum(nowCandles) || !isNum(candles) || !isNum(days) || nowCandles == "" || candles == "" || days == "" || (candles - nowCandles).toString().indexOf("-") != -1) {
            ...
            for (let i = ia; i < days; i++) {
                ...
                if (DayOfcandles + Number(ca) >= candles) {
                    ...
                    let chDAY = days - (i + 1)
                    days = i + 1
                    list.push(`第 ${i + 1} 天: ${candles}個愛心 (+${a})`)
                    ...
                    result.innerHTML = "每天需要 " + a + " 或 " + DayOfcandles + "(主要) 個愛心/" + a * 3 + " 或 " + DayOfcandles * 3 + "(主要) 根蠟燭 " + "(比預計少 " + chDAY + " 天 - 預計到" + now + ")" + `<br>${list.join("<br>")}`
                    break;
                } else {
                    ca += Number(DayOfcandles)
                    list.push(`第 ${i + 1} 天: ${ca}個愛心 (+${DayOfcandles})`)
                }
                ...
                let now = `${nowtime.split(" ")[0]}`
                console.log(now)
                let n = DayOfcandles * 3
                result.innerHTML = "每天需要 " + DayOfcandles + " 個愛心/" + n + " 根蠟燭 " + "(預計到" + now + ")" + `<br>${list.join("<br>")}`
            }
        }
    } else {
        alert("取消成功!")
